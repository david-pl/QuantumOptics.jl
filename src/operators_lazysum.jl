...
operators.dense(op::LazySum{B1,B2,F,T}) where {B1<:Basis,B2<:Basis,F,T<:Tuple{AbstractOperator{B1,B2}}} = op.factors[1] * dense(op.operators[1])
...
function operators.gemv!(alpha, a::LazySum{B1,B2}, b::Ket{B2}, beta, result::Ket{B1}) where {B1<:Basis,B2<:Basis}
...
function operators.gemv!(alpha, a::Bra{B1}, b::LazySum{B1,B2}, beta, result::Bra{B2}) where {B1<:Basis,B2<:Basis}
...
function operators.gemm!(alpha, a::LazySum{B1,B2}, b::DenseOperator{B2,B3}, beta, result::DenseOperator{B1,B3}) where {B1<:Basis,B2<:Basis,B3<:Basis}
    operators.gemm!(alpha*a.factors[1], a.operators[1], b, beta, result)
    for i=2:length(a.operators)
        operators.gemm!(alpha*a.factors[i], a.operators[i], b, 1, result)
    end
end
